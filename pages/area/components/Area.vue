...
<template>
  <div>
    ...
    <b-container class="inside bg-gray">
      ...
      <b-row align-h="start" class="content-wrap">
        <b-col
          lg="4"
          md="6"
          sm="2"
        ...
        >
          <div class="img-wrap">
            <div class="badge-new" v-if="item.isNew">nuevo</div>
            <router-link
              :to="`/contenido/${convertirSlug(item.titulo)}`"
            >
              <b-img-lazy
          ...
          <b-row align-h="between" class="info-details">
            <b-col lg="6" md="5" sm="6" class="">
              <span class="like">
                <svg
                  width="12"
                  height="12"
            ...
            <b-col lg="4" md="5" sm="6" class="obtain">
              <b-dropdown variant="transparent" class="text-dark">
                <template #button-content>
                  <b-icon icon="share" aria-hidden="true"></b-icon>
                </template>
                <b-dropdown-item-button @click="sharedFB(item)">
                  <b-icon icon="facebook" aria-hidden="true"></b-icon>
                  Facebook <span class="sr-only"></span>
                </b-dropdown-item-button>
                <b-dropdown-item-button @click="sharedTW(item)">
                  <b-icon icon="twitter" aria-hidden="true"></b-icon>
                  Twitter <span class="sr-only"></span>
                </b-dropdown-item-button>
                <b-dropdown-item-button @click="copyLink(item)">
              ...
              <span class="download">
                <a :href="item.pdf.url" target="_blank">
                  <b-icon icon="download" scale="2" aria-hidden="true"></b-icon>
                </a>
              </span>
            </b-col>
          </b-row>
          <h5 class="title-similar">{{ item.titulo }}</h5>
          <p class="info-similar" v-html="item.descripcion"></p>
        </b-col>
      </b-row>
    </b-container>
  </div>
...
export default {
  ...
  methods: {
    ...
    convertirSlug,
    //https://proyectoshm.com/medpoint/contenido/${this.convertirSlug(item.titulo)}
    sharedFB(item) {
    ...
    async addFav(contenido) {
      if (this.logged) {
        if (contenido.agregado) {
          this.favoritos = this.favoritos.filter(
            (item) => item != contenido.uid
          );
          let array = [];
          this.contenidos_array.forEach((item) => {
            if (item.uid == contenido.uid) {
              delete item.agregado;
            }
            array.push(item);
          });
          this.contenidos_array = array;
          //console.log(this.contenidos_array);
          this.user_contenido.favoritos = this.favoritos;
          //console.log(this.user_contenido)
          await this.addFavorito(this.user_contenido);
        } else {
          this.favoritos = this.favoritos.concat([contenido.uid]);
          let array = [];
          this.contenidos_array.forEach((item) => {
            if (item.uid == contenido.uid) {
              item.agregado = true;
  ...
  async created() {
    ...
    this.categoria = this.categorias.filter((item) => this.convertirSlug(item.nombre) == this.id);
    //console.log("🚀 ~ file: Area.vue ~ line 239 ~ created ~ this.categoria", this.categoria)
    ...
    if (this.logged) {
      let contenidos = this.contenidos.filter(
        (item) =>
          item.area_terapeutica == this.categoria.uid 
      );
      contenidos.filter((item) => {
        if (this.user_favoritos.includes(item.uid)) {
          item.agregado = true;
        }
        this.contenidos_array.push(item);
      });
    } else {
      let contenidos = this.contenidos.filter(
        (item) => item.area_terapeutica == this.categoria.uid &&
          item.contenido == "paciente"
      );
      contenidos.filter((item) => {
        if (this.user_favoritos.includes(item.uid)) {
          item.agregado = true;
